...
I tweak birthrate parameters by installing mod [Elder Kings fixes and tweaks](https://github.com/krisk0/ek_fixes_and_tweaks). As a result, most landed characters that successfully defend their realm have dozens of childrens, approximately one baby per 16 months. The rate is even greater if culture/religion/dynasty gives a bonus to fertility. This is unrealistic and makes game difficult to play.
...
This mod (Humans are not Rabbits) gives mother two temporary modifiers on childbirth that decrease fertility. So women are less likely to get pregnant in 12 month again.
...
I decided to turn off the married woman debuf, so in my games event fertility equals marriage fertility: `NChildbirth::MOTHER_FERTILITY_REDUCTION_PER_PREVIOUS_CHILD=0.0`.
